...
class facemesh(object):
    def __init__(self):
        self.core = Core()
        self.model = self.core.read_model("/Users/justinlien/Desktop/camera-flask/face_landmark/facemesh.xml")
        self.compiled_model = self.core.compile_model(self.model, "AUTO")
    ...
    def produce(self,image):
        ...
        results = self.compiled_model.infer_new_request({0: x})
        detections = list(results.values())[0][0]
        #detections[0:-1:3] *= 192.0
        #detections[1:-1:3] *= 192.0
        return detections
    def preprocess(self, x):
        """Converts the image pixels to the range [-1, 1]."""
        return x.float() / 127.5 - 1.0
